...
<%
	...
	if(iterate.isEmpty()) {
%>
	<display:table name="${iterate}" id="row" requestURI="${requestURI}" pagesize="${pagesize}" class="table table-hover"></display:table>
<%
	} else {
		...
		for(Field e : iterate_fields) {
			...
			if(obj != null && field_value_map.containsKey(e.getName())) {
				...
				if(field_value_map.get(e.getName()).containsKey(value)) {
					style = String.format("color:%s;", field_value_map.get(e.getName()).get(value.toString()));
				}
			}
